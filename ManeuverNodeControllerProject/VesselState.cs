...
namespace MuMech
{
    public class VesselState
    {
        ...
        private delegate double FARVesselDelegate(VesselComponent v);
        ...
        private delegate void FARCalculateVesselAeroForcesDelegate(VesselComponent vessel, out Vector3 aeroForce, out Vector3 aeroTorque, Vector3 velocityWorldVector, double altitude);
        private static FARCalculateVesselAeroForcesDelegate FARCalculateVesselAeroForces;

        private VesselComponent vesselRef = null;
            ...
            if (isLoadedFAR)
            {
                ...
                foreach (var name in farNames)
                {
                    var methodInfo = ReflectionUtils.getMethodByReflection(
                        ...
                        new Type[] { typeof(VesselComponent) }
                    );
                    if (methodInfo == null)
                    {
                ...
                var FARCalculateVesselAeroForcesMethodInfo = ReflectionUtils.getMethodByReflection(
                    ...
                    BindingFlags.Public | BindingFlags.Static,
                    new Type[] { typeof(VesselComponent), typeof(Vector3).MakeByRefType(), typeof(Vector3).MakeByRefType(), typeof(Vector3), typeof(double) }
                );
        ...
        //}
        public bool Update(VesselComponent vessel)
        {
            if (time == GameManager.Instance.Game.UniverseModel.UniversalTime)
                return true;
        ...
        private void TestStuff(VesselComponent vessel)
        {
        ...
        void UpdateVelocityAndCoM(VesselComponent vessel)
        {
        ...
        void UpdateBasicInfo(VesselComponent vessel)
        {
            time = GameManager.Instance.Game.UniverseModel.UniversalTime;
        ...
        void UpdateRCSThrustAndTorque(VesselComponent vessel)
        {
        ...
        private void CalculateVesselAeroForcesWithCache(VesselComponent v, out Vector3 farForce, out Vector3 farTorque, Vector3d surfaceVelocity, double altitudeASL) 
        {
        ...
        void AnalyzeParts(VesselComponent vessel, EngineInfo einfo, IntakeInfo iinfo)
        {
            parachutes.Clear();
        ...
        void ToggleRCSThrust(VesselComponent vessel)
        {
        ...
        void UpdateMoIAndAngularMom(VesselComponent vessel)
        {
            MoI = vessel.MOI;
        ...
        public class EngineInfo
        {
            ...
            public void Update(Vector3d c, VesselComponent vessel)
            {
